...
export default function StockMovements() {
  const [currentFilter, setCurrentFilter] = useState<'All' | 'Inbound' | 'Outbound' | 'Adjustments'>('All');
  ...
  const stockLogs: StockLogItem[] = [
    {
      ...
      sku: 'VEG-042-KL',
      imageUrl: 'https://images.unsplash.com/photo-1628773822503-930a84001a1a?w=120&auto=format&fit=crop&q=60',
      type: 'Inbound',
      quantity: 150,
      before: 12,
      after: 162
    ...
    },
    {
      ...
      sku: 'VEG-110-TM',
      imageUrl: 'https://images.unsplash.com/photo-1595855759920-86582396756a?w=120&auto=format&fit=crop&q=60',
      type: 'Adjustment',
      quantity: -8,
      before: 32,
      after: 24
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2.5 pl-1">
              ...
              <div className="leading-tight">
                ...
                <p className="text-[9px] font-bold text-[#7C8465] uppercase tracking-wider">Manager</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="p-8 max-w-7xl w-full mx-auto space-y-6 flex-1">
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="bg-white rounded-2xl border border-[#E5E9D5] p-5 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-[#7C8465] uppercase tracking-wider">Total Stock Value</p>
                <h3 className="text-[26px] font-black text-[#1E2516] tracking-tight">$248,390.00</h3>
                <p className="text-[10px] font-bold text-[#40AA22] flex items-center gap-1">
                  <span>↗ +4.2%</span> <span className="text-[#7C8465] font-semibold">from last month</span>
                </p>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-[#E5E9D5] p-5 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-[#7C8465] uppercase tracking-wider">Restocks This Month</p>
                <h3 className="text-[26px] font-black text-[#1E2516] tracking-tight">1,204</h3>
                <p className="text-[10px] font-bold text-[#4D5335] flex items-center gap-1.5">
                  <DeliveryIcon className="w-3.5 h-3.5" /> <span>24 Pending deliveries</span>
                </p>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-[#E5E9D5] p-5 shadow-sm flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-[#7C8465] uppercase tracking-wider">Waste/Returns</p>
                <h3 className="text-[26px] font-black text-[#FF5C5C] tracking-tight">-$2,140.45</h3>
                <p className="text-[10px] font-bold text-[#FF5C5C] flex items-center gap-1">
                  <span>⚠️ +12%</span> <span className="text-[#FF7A7A] font-semibold">vs expected</span>
                </p>
              </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-1">
            <div className="flex items-center bg-[#EAEEDC]/40 border border-[#E1E6CE] p-1 rounded-xl">
              <FilterTab label="All Movements" active={currentFilter === 'All'} onClick={() => setCurrentFilter('All')} />
              <FilterTab label="Inbound" active={currentFilter === 'Inbound'} onClick={() => setCurrentFilter('Inbound')} />
              <FilterTab label="Outbound" active={currentFilter === 'Outbound'} onClick={() => setCurrentFilter('Outbound')} />
              <FilterTab label="Adjustments" active={currentFilter === 'Adjustments'} onClick={() => setCurrentFilter('Adjustments')} />
            </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E5E9D5] shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left text-sm">
                ...
                <tbody className="divide-y divide-[#F0F3E6] font-semibold text-[#2D311B]">
                  {stockLogs.map((log) => (
                    <tr key={log.id} className="hover:bg-[#F9FBF3]/30 transition-colors">
                      {/* DATE LOG TIMESTAMP MODULE */}
                      <td className="whitespace-nowrap px-6 py-4">
                        <p className="text-xs font-black text-[#1E2516]">{log.date}</p>
                        <p className="text-[10px] font-bold text-[#7C8465] mt-0.5">{log.time}</p>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <div>
                            <p className="text-xs font-black text-[#1E2516]">{log.productName}</p>
                            <p className="text-[10px] font-bold text-[#7C8465] uppercase tracking-wider mt-0.5">
                              SKU: <span className="font-mono text-[#4D5335]">{log.sku}</span>
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className={`whitespace-nowrap px-6 py-4 text-center text-xs font-black ${
                        log.quantity > 0 ? 'text-[#40AA22]' : log.type === 'Adjustment' ? 'text-[#FF5C5C]' : 'text-[#2D311B]'
                      }`}>
                        {log.quantity > 0 ? `+ ${log.quantity}` : log.quantity.toString().replace('-', '- ')}
                      </td>
            ...
            <div className="bg-[#F9FBF3]/50 border-t border-[#E5E9D5] px-6 py-4 flex items-center justify-between text-xs font-bold text-[#7C8465]">
              <div>
                Showing <span className="text-[#1E2516]">1 to 10</span> of <span className="text-[#1E2516]">4,230</span> entries
              </div>
...
function SidebarItem({ icon, active = false }: SidebarItemProps) {
  return (
    <button className={`h-11 w-11 rounded-xl flex items-center justify-center transition-all ${
      active ? 'bg-[#C1F161] text-[#1E2516]' : 'text-[#BAC29E] hover:bg-white/5 hover:text-white'
    }`}>
      {icon}
    </button>
  );
...
const WalletIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M21 12a2.25 2.25 0 0 0-2.25-2.25H15a3 3 0 1 1-6 0H5.25A2.25 2.25 0 0 0 3 12m18 0v6A2.25 2.25 0 0 1 18.75 20.25H5.25A2.25 2.25 0 0 1 3 18.125V12m18 0c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125m18 0v-1.5A2.25 2.25 0 0 0 18.75 8.25h-13.5A2.25 2.25 0 0 0 3 10.5V12" />
  </svg>
);
...
const SlidersIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M6 13.5V3.75m0 9.75a1.5 1.5 0 1 1 0 3m0-3a1.5 1.5 0 1 0 0 3m0 3.75V16.5m12-3V3.75m0 9.75a1.5 1.5 0 1 1 0 3m0-3a1.5 1.5 0 1 0 0 3m0 3.75V16.5m-6-9V3.75m0 3.75a1.5 1.5 0 1 1 0 3m0-3a1.5 1.5 0 1 0 0 3m0 9.75V10.5" />
  </svg>
);
...
const DeliveryIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124l-.321-5.128a2.25 2.25 0 0 0-2.236-2.112h-4.5a1.125 1.125 0 0 0-1.125 1.125v9.75M8.25 18.75h6.75M12 14.25H3.375M12 14.25V3.375a1.125 1.125 0 0 1 1.125-1.125h1.5a1.125 1.125 0 0 1 1.125 1.125v2.25m3 3h1.5c.621 0 1.125.504 1.125 1.125v1.5a1.125 1.125 0 0 1-1.125 1.125H16.5" />
  </svg>
);
...
const SidebarArrowsIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
  </svg>
);
const SidebarTruckIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124l-.321-5.128a2.25 2.25 0 0 0-2.236-2.112h-4.5a1.125 1.125 0 0 0-1.125 1.125v9.75" />
  </svg>
);
const SidebarReceiptIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
  </svg>
);
const SidebarProfileIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M17.982 18.725A7.488 7.488 0 0 0 12 15.75a7.488 7.488 0 0 0-5.982 2.975m11.963 0a9 9 0 1 0-11.963 0m11.963 0A8.966 8.966 0 0 1 12 21a8.966 8.966 0 0 1-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
  </svg>
);
const SidebarLogoutIcon = (props: SVGProps) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" {...props}>
